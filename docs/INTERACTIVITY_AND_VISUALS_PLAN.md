...
- Three small, original MP3 files play correct, wrong, and completion cues. `js/effects.js` keeps the existing Web Audio oscillator as a fallback if a sample fails; mute pauses active samples, stops and disconnects queued fallback tones, and suspends the fallback context. Muted tones cannot resume during a later cue. The sample source is reproducible from `tools/generate_feedback_audio.py`.
