...
- Scores are completed game attempts kept apart from lesson source content. Flashcard passes track cards viewed without claiming answer accuracy. Wrong answers lead to a retry or review opportunity.
...
Learner progress uses a separate localStorage key and retains up to 500 completed attempts across lessons. Each entry records lesson ID, activity, language direction, score or unscored flashcard pass, total, and completion time. The per-lesson view shows a summary and five recent attempts. Deleting a lesson or importing a replacement collection removes orphaned results; clearing progress does not edit the lesson. Progress is not part of lesson JSON backups. If local progress storage fails, the game remains playable and the current session's results can still appear until the page closes.
...
| 4. More practice | Memory, true/false, categories, sentence order, sentence completion, picture labels. | All six activities have code and local checks; Windows browser acceptance remains open. |
| 5. Quality and progress | Learner progress, retries, backups, accessibility, visual/audio polish, complete browser acceptance pass. | Local progress tracking has code and checks; the full Windows offline browser acceptance pass remains open. |
