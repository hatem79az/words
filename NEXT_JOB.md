...
## Next deliverable: true or false

Build a true or false recognition activity from the same saved lesson pairs. Show a prompt and proposed answer in the selected languages; ask whether they belong together. Use deliberately balanced correct and incorrect rounds rather than random guessing with a strong bias.

Acceptance checks:

1. At least four unambiguous pairs are needed; the readiness message explains when a lesson is too small.
2. Correct and incorrect proposals appear in a balanced set, and each incorrect answer comes from another eligible item with a different written term.
3. True/false buttons are keyboard accessible. Feedback identifies the real matching answer after an error, tracks score accurately, and supports all four language directions.
...
Finish picture labels, category sort, sentence order, and sentence completion; then learner progress and the complete browser acceptance pass. **One-click Windows packaging is phase 2 only.**
